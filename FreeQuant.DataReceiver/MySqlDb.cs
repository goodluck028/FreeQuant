    ...
    internal class MySqlDb : IDataBase
    {
        ...
        public void CreateDb()
        {
            ...
            try
            {
                ...
                cmd.Connection = conn;
                //建基础数据库
                cmd.CommandText = "CREATE SCHEMA If Not Exists `base_future` ;";
                cmd.ExecuteNonQuery();
                //建历史数据库
                cmd.CommandText = "CREATE SCHEMA If Not Exists `hisdata_future` ;";
                cmd.ExecuteNonQuery();
                //将连接添加回连接池中
        ...
        public void CreateTable(string product)
        {
            ...
            try
            {
                ...
                cmd.Connection = conn;
                //创建合约表
                cmd.CommandText = @"CREATE  TABLE If Not Exists `base_future`.`t_instrument` (
                                      `instrument_id` VARCHAR(16) NOT NULL ,
                                      `product_id` VARCHAR(16) NOT NULL ,
                                      `exchange_id` VARCHAR(16) NOT NULL ,
                                      `volume_multiple` INT NOT NULL ,
                                      `price_tick` FLOAT NOT NULL ,
                                      `max_order_volume` INT NOT NULL ,
                                        PRIMARY KEY (`instrument_id`))
                                    ENGINE = MyISAM;";
                cmd.ExecuteNonQuery();
                //创建一分钟bar表
                cmd.CommandText = $@"CREATE TABLE If Not Exists `hisdata_future`.`t_bar1min_{product}` (
                                  `exchange_id` varchar(16) NOT NULL,
                                  `instrument_id` varchar(16) NOT NULL,
                ...
                cmd.ExecuteNonQuery();
                //创建tick表
                cmd.CommandText = $@"CREATE  TABLE If Not Exists `hisdata_future`.`t_tick_{product}` (
                          `exchange_id` VARCHAR(16) NOT NULL ,
                          `instrument_id` VARCHAR(16) NOT NULL ,
                          `multiplier` FLOAT NOT NULL ,
        ...
        //写入合约
        public void InsertInstrument(Instrument inst)
        {
            MySqlConnection conn = ConnectionPool.getPool().getConnection();
            try
            {
                string sql = @"INSERT IGNORE INTO `base_future`.`t_instrument`
                                (`instrument_id`
                                ,`product_id`
                                ,`exchange_id`
                                ,`volume_multiple`
                ...
                MySqlCommand cmd = new MySqlCommand(sql, conn);
                //
                cmd.Parameters.AddWithValue("@instrument_id", inst.InstrumentID);
                cmd.Parameters.AddWithValue("@product_id", inst.ProductID);
                cmd.Parameters.AddWithValue("@exchange_id", inst.Exchange.ToString());
                cmd.Parameters.AddWithValue("@volume_multiple", inst.VolumeMultiple);
                cmd.Parameters.AddWithValue("@price_tick", inst.PriceTick);
                cmd.Parameters.AddWithValue("@max_order_volume", inst.MaxOrderVolume);
                //
                cmd.ExecuteNonQuery();
                //将连接添加回连接池中
                ConnectionPool.getPool().closeConnection(conn);
        ...
        public List<Instrument> ReadInstruments()
        {
            ...
            try
            {
                string sql = @"SELECT
                                ...
                                `price_tick`,
                                `max_order_volume`
                            FROM `base_future`.`t_instrument`;";
                MySqlCommand cmd = new MySqlCommand(sql, conn);
                MySqlDataReader reader = cmd.ExecuteReader();
                while (reader.Read())
                {
                    string instrument_id = reader["instrument_id"] as string;
                    string product_id = reader["product_id"] as string;
                    Exchange exchange = Exchange.Unknown;
                    switch (reader["exchange_id"] as string)
                    {
                        case "SHFE":
                    ...
                    int volume_multiple = (int)reader["volume_multiple"];
                    double price_tick = (double)reader["price_tick"];
                    int max_order_volume = (int)reader["max_order_volume"];
                    Instrument instrument = new Instrument(instrument_id,product_id,exchange,volume_multiple,price_tick,max_order_volume);
