    ...
    internal class DataReceiver
    {
        ...
        public void run()
        {
            ...
            MdBroker.OnStatusChanged += (ConnectionStatus mdStatus) =>
            {
                ...
                {
                    //首先从数据库加载合约
                    foreach(Instrument inst in mDb.ReadInstruments())
                    {
                        _subInstrument(inst);
                    }
                    //当行情服务器连接成功时，连接交易服务器
                    TdBroker.OnInstrument += _savInstrument;
                    TdBroker.OnInstrument += _subInstrument;
                    TdBroker.OnStatusChanged += (ConnectionStatus tdStatus) =>
                    {
                        if (tdStatus == ConnectionStatus.Connected) //连接成功
                        {
                            //查询合约
        ...
        //保存合约
        private void _savInstrument(Instrument inst)
        {
            mDb.InsertInstrument(inst);
        }

        //订阅合约
        private string[] mProducts = Config.Instruments.Split(',');
        private HashSet<Instrument> mInstruments = new HashSet<Instrument>();
        private void _subInstrument(Instrument inst)
        {
            //建表
            foreach (string product in mProducts)
            {
                ...
                {
                    mDb.CreateTable(inst.ProductID);
                    //触发合约事件
